...
def expand_vector_columns(logging_results, REFERENCE_MODELS):
    """
    expanding the logged results dictionary with vector columns into a pandas DataFrame.
    """
    rows = []
    for i in range(len(logging_results['epoch'])):
        row = {
            'epoch': logging_results['epoch'][i],
            'batch': logging_results['batch'][i],
            'batch_acc_pre': logging_results['batch_acc_pre'][i],
            'batch_acc_post': logging_results['batch_acc_post'][i],
            'val_acc': logging_results['val_acc'][i],
            'test_acc': logging_results['test_acc'][i],
            'loss': logging_results['loss'][i],
        }
        if logging_results['alphas'][i] is not None:
            alpha_vec = logging_results['alphas'][i]
            for j, ref in enumerate(REFERENCE_MODELS):
                row[f"alpha_{ref}"] = float(alpha_vec[j])
        else:
            for ref in REFERENCE_MODELS:
                row[f"alpha_{ref}"] = None
        if logging_results['dpo_by_refs'][i] is not None:
            dpo_vec = logging_results['dpo_by_refs'][i]
            for j, ref in enumerate(REFERENCE_MODELS):
                row[f"dpo_{ref}"] = float(dpo_vec[j])
        else:
            for ref in REFERENCE_MODELS:
                row[f"dpo_{ref}"] = None
        rows.append(row)
